...
We need to make sure we are loading the file from the right directory. Replace the `with open("todo_list.txt", "r") as file:` statement in the `app.py` file with the following code:
